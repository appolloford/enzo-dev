/***********************************************************************
/
/  GRID CLASS (WRAP THE GRACKLE TEMPERATURE SOLVER)
/
/  written by: Britton Smith (wrote Grid_GrackleWrapper.C on which this is based)
/  modified by: Duncan Christie 
/  date:       July 2016
/  modified1: The code to Grid_GrackleWrapper.C was modified to call the Grackle
/             temperature routines instead of the cooling routines.  
/
/  PURPOSE: Call Grackle to get the Grackle routine calculate_temperature()
/
/  RETURNS:
/    SUCCESS or FAIL
/
...
int grid::GrackleTemperatureWrapper(float *temperature)
{
  ...
  LCAPERF_START("grid_GrackleTemperatureWrapper");
  ...
  /* Call the temperature solver. */
  //  if (calculate_temperature(&grackle_units, (double) afloat,
  ...
  //			    MetalPointer,temperature) == FAIL) {
  if (calculate_temperature(&grackle_units, &my_fields, temperature) == FAIL ) {
  ...
  LCAPERF_STOP("grid_GrackleTemperatureWrapper");
